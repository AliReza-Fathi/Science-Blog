class MyStrings {
  static const String scienceBlogGithubUrl =
      "https://github.com/AliReza-Fathi/Science-Blog";
  static const String viewHotestBlog = "مشاهده داغ ترین نوشته ها ";
  static const String viewHotestPodCasts = "مشاهده داغ ترین پادکست ها";
  static const String relatedArticle = "نوشته های مرتبط";
  static const String submitArticle = "ارسال مقاله";
  static const String submitPodcasts = "ارسال پادکست";
  static const String imageProfileEdit = "ویرایش عکس پروفایل";
  static const String myFavBlog = "مقالات مورد علاقه من";
  static const String managmentArticle = "مدیریت مقاله ها";
  static const String managmentPodcast = "مدیریت پادکست ها";
  static const String myFavPodcast = "پادکست های مورد علاقه من";
  static const String logOut = "خروج از حساب کاربری";
  static const String newBlogTitleText =
      "دونسته هات رو با بقیه به اشتراک بزار ...";
  static const String insertYourEmail = "لطفا ایمیلت رو وارد کن";
  static const String activateCode = "کد فعال سازی رو وارد کن ";
  static const String chooseCats = "دسته بندی هایی که دوست داری رو انتخاب کن";
  static const String welcom = """به ساینس بلاگ خوش اومدی

برای ارسال مطلب و پادکست باید حتما 
ثبت نام کنی """;
  static const String shareText = """

من از ساینس بلاگ استفاده میکنم ، خیلی باحال و مفیده ، تو هم داشته باش\n
""";
  static const String aboutUs = """
اپلیکیشن ساینس بلاگ به عنوان موضوع پروژه
درجه کارشناسی رشته مهندسی نرم افزار کامپیوتر 
در دانشکده امام محمد باقر ساری
توسط دانشجویان
علیرضا فتحی آراللوی بزرگ
مهدی رفیع زاده تولون
با راهنمایی استاد دکتر محمد امیری
در تابستان 1401 برنامه نویسی شده است.""";

  static const String successfulRegistration = """
تبریک میگم ، ایمیل با موفقیت تایید شد
لطفا اطلاعات ثبت نام رو کامل کن
""";

  static const String newBlogBodyText =
      """فکر کن! اینجا بودنت به این معناست که یک گیک تکنولوژی هستی
دونسته هات رو با جامعه‌ گیک های فارسی زبان به اشتراک بذار...""";
  static const String manageMyArticles =
      """هنوز هیچ مقاله ای به جامعه گیک های فارسی 
اضافه نکردی !""";
}
